...
export function ExplorePage() {
    ...
    return (
        <div className="bg-[#050507] min-h-screen flex flex-col bg-fixed bg-center bg-cover scroll-smooth" >
            ...
            {/* Mobile Menu Button */}
            <button 
                className="md:hidden text-xl text-[#000] "
            ...
            <div className={`absolute top-[60px] left-0 w-full bg-[#000]/90 backdrop-blur-lg flex flex-col items-center gap-4 p-4 transition-all duration-300 ${isOpen ? "block" : "hidden"} md:hidden`}>
                <span className="cursor-pointer hover:opacity-80 transition-opacity">Batches</span>
                <span className="cursor-pointer hover:opacity-80 transition-opacity">Testnet</span>
            ...
            <div className="mt-[120px] text-center px-4">
                ...
                <div className="explore-content container mx-auto mt-10 flex flex-col items-center">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-[1000px]">
                        {[{
                            ...
                            <div key={index} className="flex justify-between p-4 items-center gap-5 w-full bg-[#000] rounded-xl shadow-lg">
                                <p className="text-2xl">{isshow && typeof item.value === 'number' ? <CountUp end={item.value} duration={5} /> : item.value}</p>
                    ...
                    <div className="w-full max-w-[700px] bg-[#0c0c10] flex justify-center items-center rounded-xl shadow-lg mt-5 p-3 text-white text-sm sm:text-base">
                    ...
                    <div className="w-full max-w-[700px] bg-[#000] mt-5 p-4 rounded-xl border border-[#312e2e] mb-10">
                        <ul className="text-white flex flex-col gap-3">
                            {["dsadsad", "1244", "sdadsada", "24141", "d223rsad"].map((item, index) => (
                                <li key={index} className="text-xl flex justify-between items-center p-2 rounded-lg border border-transparent hover:border-[#433b45] transition-all cursor-pointer duration-200">
                                    {item}
